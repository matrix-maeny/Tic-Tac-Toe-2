...
import com.matrix_maeny.tictactoe2.Positions;
import com.matrix_maeny.tictactoe2.R;
import com.matrix_maeny.tictactoe2.databinding.FragmentSinglePlayerBinding;
...
public class SinglePlayerFragment extends Fragment {
    ...
    FragmentSinglePlayerBinding binding;
    ...
    int i = 0, pivot = 0;
    int preRow, preColumn,turner=1;
    int player1Score = 0, player2Score = 0;
    boolean restartChecker = false;
    boolean gameStatus = true, computerWinStatus = false, computerTurn = false, colorSign = false, sounds = true;
    boolean turnChanger = false;
    ImageView[][] matrixViews = null;

    MediaPlayer player = null;
    ArrayList<Positions> list;
    ArrayList<Positions> list2;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container,
        ...
        return binding.getRoot();//inflater.inflate(R.layout.fragment_single_player, container, false);
    }
    ...
    CompoundButton.OnCheckedChangeListener switchListener = (buttonView, isChecked) -> sounds = isChecked;
    ...
    View.OnClickListener restartGameListener = v -> {
        if (restartChecker) {
            if (!computerTurn) {
                if (!colorSign)
                    restartGame();
                else colorSign = false;
            } else {
                Toast.makeText(getContext(), "Wait...Click again", Toast.LENGTH_SHORT).show();
                computerTurn = false;
            }
        } else {
    ...
    final void checkResults(int row, int column) {
        ...
        preRow = row;
        preColumn = column;
        list2.add(new Positions(row, column));
    ...
    final void takeTurn() {
        ...
        boolean checkPointStatus = false;
        turnChanger = true;
        boolean matched = false, match1, match2, match3;
        int r1, r2, r3, r5;
        int c1, c2, c3, c5;
        r2 = r3 = r5 = 0;
        c2 = c3 = c5 = 0;
        ...
        for (int i = 0; i < 12; i++) {
            for (int j = 0; j < 9; j++) {
                ...
                if (matrix[i][j] == 0) {
                    if (checkComputerWinStatus(i, j)) {
                        checkPointStatus = true;
                        break abc;
                    }
                }
            }
        } // taking chances of win 1

        if (!checkPointStatus) {
            if (list.size() != 0) {
                for (int i = list.size() - 1; i >= 0; i--) {
                    if (checkComputerWinStatus2(list.get(i).getRow(), list.get(i).getColumn())) {
                        checkPointStatus = true;
                        break;
                    }
                }
            }
        } // taking chances of win 2
        ...
        // checking is there any combinations matching by the user side

        int row;
        int column;
        if (!checkPointStatus) { // checking from center of the point he kept

            r1 = c1 = 0;
            abc:
            for (int j = list2.size() - 1; j >= 0; j--) {
                row = list2.get(j).getRow();
                column = list2.get(j).getColumn();
                for (int i = 1; i <= 4; i++) {
...
//                    matched = (matrix[r1][c1] != 0 && matrix[r2][c2] != 0 && matrix[r3][c3] != 0) && (matrix[r1][c1] == matrix[r2][c2] && matrix[r2][c2] == matrix[r3][c3]);
                        matched = ((matrix[r1][c1] != 0 && matrix[r2][c2] != 0) || (matrix[r2][c2] != 0 && matrix[r3][c3] != 0) || (matrix[r1][c1] != 0 && matrix[r3][c3] != 0));// ((matrix[r1][c1] != -1 && matrix[r2][c2] != -1) || (matrix[r2][c2] != -1 && matrix[r3][c3] != -1) || (matrix[r1][c1] != -1 && matrix[r3][c3] != -1));
                        match2 = (matrix[r1][c1] == 1 && matrix[r1][c1] == matrix[r2][c2] || matrix[r1][c1] == matrix[r3][c3] || matrix[r2][c2] == matrix[r3][c3]);
                        match3 = (matrix[r1][c1] == -1 || matrix[r2][c2] == -1 || matrix[r3][c3] == -1);

                        matched = matched && match2;
                        ...
                        if (matched) {
                            ...
                            if (matrix[r1][c1] == 0) {
                                list.add(new Positions(r1, c1));
                                setThePoint(r1, c1);
                                checkWinStatus(r1, c1);
                            } else if (matrix[r2][c2] == 0) {
                                list.add(new Positions(r2, c2));
                                setThePoint(r2, c2);
                                checkWinStatus(r2, c2);

                            } else if (matrix[r3][c3] == 0) {
                                list.add(new Positions(r3, c3));
                                setThePoint(r3, c3);
                                checkWinStatus(r3, c3);
        ...
        } // Checking from the selected of the box: defend 2
        ...
        if (!checkPointStatus) {
            ...
            for (int j = list2.size() - 1; j >= 0; j--) {
                ...
                for (int i = 1; i <= 8; i++) { // checking from the selected box of selected box

                    switch (i) {
                        case 1:                                 // East to west: rows are constant and columns will decrease ( last one will increase)-- east side
                            r2 = r3 = r5 = row;
                            ...
                            break;
                        case 2:                                // exactly opposite(line) to the case 1: rows are constant but columns will increase -- west side
                            r2 = r3 = r5 = row;
                            ...
                            break;
                        case 3:
                            c2 = c3 = c5 = column;  // north to south: columns are constant, rows will decrease -- north side
                            ...
                            break;
                        case 4:
                            c2 = c3 = c5 = column; // opposite line: columns are constant, rows will increase: -- south side
                            ...
                            break;
                        case 5:                             // North-East: row are columns will decrease
                            ...
                            break;
                        case 6:                             // south-west: rows and columns will increase
                            ...
                            break;
                        case 7:                         // North-west: rows will decrease and columns will increase
                            ...
                            break;
                        case 8:                         // South-East: rows will increase, columns will decrease
...
//condition 1
                    try {
                        if (matrix[row][column] == -1) {
                            match1 = (matrix[r2][c2] == -1 && matrix[r3][c3] == 0) || (matrix[r2][c2] == 0 && matrix[r3][c3] == -1); // you should place at r2 or r3;
                            match2 = matrix[r5][c5] != -1;
                            matched = match1 && match2;
                        } else if (matrix[row][column] == 1) {
                            match1 = (matrix[r2][c2] == -1 && matrix[r3][c3] == 0) || (matrix[r2][c2] == 0 && matrix[r3][c3] == -1);
                            match2 = (matrix[r2][c2] == 1 && matrix[r3][c3] == 0) || (matrix[r2][c2] == 0 && matrix[r3][c3] == 1);
                            matched = match1 || match2;
                    ...
                    } catch (Exception ignored) { // condition 1
                    }

                    // condition 2
                    try {
                        if (matrix[row][column] == -1) {
                            matched = (matrix[r2][c2] == 0 && matrix[r5][c5] == -1) || (matrix[r2][c2] == -1 && matrix[r5][c5] == 0);
                        } else if (matrix[row][column] == 1) {
                            match1 = (matrix[r2][c2] == 0 && matrix[r5][c5] == -1) || (matrix[r2][c2] == -1 && matrix[r5][c5] == 0);
                            match2 = (matrix[r2][c2] == 0 && matrix[r5][c5] == 1) || (matrix[r2][c2] == 1 && matrix[r5][c5] == 0);
                        ...
                        if (matched) {
                            if (matrix[r2][c2] == 0) {
                                ...
                                setThePoint(r2, c2);
                                checkWinStatus(r2, c2);
                            } else if (matrix[r5][c5] == 0) {
                                list.add(new Positions(r5, c5));
                                setThePoint(r5, c5);
                                checkWinStatus(r5, c5);
        ...
        // taking chance for computer side if nothing matches
        r2 = r3 = c2 = c3 = 0;
        if (!checkPointStatus) {
            ...
            for (int i = list.size() - 1; i >= 0; i--) {
                ...
                int tempColumn = list.get(i).getColumn();
                ...
                for (int j = 1; j <= 4; j++) {
                    switch (j) {
                        ...
                        case 3:
                            ...
                            c2 = c3 = tempColumn; // column is constant;
                            break;
                        case 4:
                            // rows and columns will change
                            r2 = tempRow - 1;
                    ...
                    try {
                        notOccupied = matrix[r2][c3] == 0 && matrix[r3][c3] == 0;
        ...
        // taking random box to fill by the computer.
        if (!checkPointStatus) {
            Random random = new Random();
            int randomRow, randomColumn;

            do { // getting random row and column

                randomRow = random.nextInt(12);
                randomColumn = random.nextInt(9);

            } while (matrix[randomRow][randomColumn] != 0);

            list.add(new Positions(randomRow, randomColumn));
            setThePoint(randomRow, randomColumn);
            checkWinStatus(randomRow, randomColumn);
        }
...
//                && (matrix[r1][c1] != -1 && matrix[r2][c2] != -1 && matrix[r3][c3] != -1)
        ...
        for (int i = 1; i <= 4; i++) {//checking from the  selected box assuming it as it is in the center between two boxes
            switch (i) {
                case 1:
                    r1 = r2 = r3 = row; // rows are equal
...
//                matched = matched || matched2;
        ...
        }//checking from the  selected box assuming it as it is in the center between two boxes
        ...
        for (int i = 1; i <= 8; i++) { // checking from the  side ways
...
//                boolean matched3 = matrixViews[r1][c1].getDrawable().getConstantState() == matrixViews[r2][c2].getDrawable().getConstantState() && matrixViews[r1][c1].getDrawable().getConstantState() == matrixViews[r3][c3].getDrawable().getConstantState();
                matched = matched2 && (matrix[r4][c4] != (-turnNumber));
            } catch (Exception e) {
                continue;
            }
        ...
        if (!checkGameStatus(matrix)) {
            if (turnChanger) {
                ...
                String text = "Cross got one more chance";

                if (turnNumber == 1) {
                    text = "You got one more chance";
                } else if (turnNumber == 2) {
                    computerWinStatus = false;
                    text = "Cross got one more chance";
                    startThread();
                }
    ...
    final boolean checkComputerWinStatus(int row, int column) {

        boolean matched, match1, match2, match3, match4, match5;
        int r2, r3, r4, r5;
        int c2, c3, c4, c5;
        r2 = r3 = r4 = r5 = 0;
        c2 = c3 = c4 = c5 = 0;
        ...
        for (int i = 1; i <= 8; i++) { // Sequential clockwise (8 loops)
            ...
            switch (i) {
                ...
                case 2:                                // exactly opposite(line) to the case 1: rows are constant but columns will increase -- west side
                    ...
                    break;
                case 3:
                    c2 = c3 = c4 = c5 = column;  // north to south: columns are constant, rows will decrease -- north side
                    ...
                    break;
                case 4:
                    c2 = c3 = c4 = c5 = column; // opposite line: columns are constant, rows will increase: -- south side
            ...
            try {
                ...
                // 1. sequential match..( 8 loops) running through clockwise direction in a matrix by taking given box(with row and column parameters) as centre.
                match1 = (matrix[r2][c2] == -2 && matrix[r3][c3] == -2) || (matrix[r2][c2] == 2 && matrix[r3][c3] == 2);
                match2 = (matrix[r2][c2] == -2 && matrix[r3][c3] == 2) || (matrix[r2][c2] == 2 && matrix[r3][c3] == -2);
                match3 = matrix[r4][c4] != -2;
                match4 = matrix[r2][c2] == -2 && matrix[r5][c5] == -2;
                match5 = (matrix[r2][c2] == -2 && matrix[r5][c5] == 2) || (matrix[r2][c2] == 2 && matrix[r5][c5] == -2);

                matched = ((match1 || match2) && match3) || match4 || match5;

                if (matched) {
                    setThePoint(row, column);
                    checkWinStatus(row, column);
                    return true;
                }
            } catch (Exception ignored) {
        ...
        } // Sequential clockwise checking  complete


        return false;
    } // checks whether the computer has a chance of win or not


    final boolean checkComputerWinStatus2(int row, int column) {
        ...
        for (int i = 1; i <= 8; i++) { // Sequential clockwise (8 loops)
            ...
            switch (i) {
                case 1:
                    r1 = r2 = r3 = r4 = r5 = row; // rows are equal
                ...
                case 3:
                    ...
                    c1 = c2 = c3 = c4 = c5 = column; // column is constant;
                    break;
                case 4:
                    r1 = row; // rows and columns will change
                    r2 = r1 - 1;
            ...
            try {
                ...
                if (matrix[r1][c1] == -2) {
                    boolean match1 = matrix[r2][c2] == 2 && matrix[r3][c3] == 0;
                    boolean match2 = matrix[r2][c2] == 0 && matrix[r3][c3] == 2;
                    boolean match3 = !(matrix[r4][c4] == -2 && matrix[r5][c5] == -2);

                    matched = match3 && (match1 || match2);
                    if (matched) {
                ...
                } else if (matrix[r1][c1] == 2) {
                    boolean match1 = matrix[r2][c2] == -2 && matrix[r3][c3] == 0;
                    boolean match2 = matrix[r2][c2] == 0 && matrix[r3][c3] == -2;

                    boolean match4 = matrix[r2][c2] == 2 && matrix[r3][c3] == 0;
                    boolean match5 = matrix[r2][c2] == 0 && matrix[r3][c3] == 2;


                    boolean match3 = !(matrix[r4][c4] == -2 && matrix[r5][c5] == -2);

                    matched = match3 && ((match1 || match2) || (match4 || match5));
                    if (matched) {
        ...
        for (int i = 1; i <= 4; i++) {//checking from the  selected box assuming it as it is in the center between two boxes
            ...
            try {
                ...
                if (matrix[r1][c1] == -2) {
                    boolean match1 = matrix[r2][c2] == 0 && matrix[r3][r4] == 2;
                    boolean match2 = matrix[r2][c2] == 2 && matrix[r3][r4] == 0;
                    boolean match3 = !(matrix[r4][c4] == -2 && matrix[r5][c5] == -2);
        ...
        for (int i = 1; i <= 8; i++) { // checking from the  side ways
            ...
            try {
                if (matrix[r1][c1] == -2) {
                    boolean match1 = matrix[r2][c2] == -2 && matrix[r3][c3] == 0;
                    boolean match3 = matrix[r2][c2] == 0 && matrix[r3][c3] == -2;
                    boolean match2 = !(matrix[r4][c4] == -2 || matrix[r5][c5] == -2);
                    matched = (match1 || match3) && match2;
                    if (matched) {
...
//
    ...
    final void setThePoint(int row, int column) {
        if (sounds) {
            playSound();
        }
        changeState(row, column);

        matrixViews[row][column].setImageResource(R.drawable.player_2);
        ...
        handler.post(new Runnable() {
            ...
            public void run() {
                colorSign = true;
                if (pivot == 1) {
                    matrixViews[row][column].setBackgroundResource(R.color.boxBackground);
                    matrixViews[row][column].setVisibility(View.VISIBLE);
                    pivot = 2;
                    handler.postDelayed(this, 400);
                } else {

                    matrixViews[row][column].setBackgroundColor(Color.TRANSPARENT);
    ...
    final void changeState(int row, int column) {
    ...
    final void restartGame() {
        ...
        if (turner == 1) {
            ...
            changeTurn();
            computerTurn = true;
            Toast.makeText(getContext(), "Wait...Now it's computer turn", Toast.LENGTH_LONG).show();
            Handler handler = new Handler();
            handler.postDelayed(this::startThread, 1000);
        } else if (turner == 0) {
            turner = 1;
            Toast.makeText(getContext(), "Now it's your turn", Toast.LENGTH_LONG).show();
        }
    }
    ...
    final void startThread() {
        ...
        handler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                binding.processView.setText(txt);
                if (i <= 4) {
                    i++;
                    handler.postDelayed(this, 390);

                } else {
                    binding.processView.setText("■ ■ ■");
                }
    ...
    final void showWinnerPoster() {
        ...
        binding.restartBtn.setVisibility(View.GONE);
        binding.turnLayout.setVisibility(View.GONE);
        binding.soundSwitch.setVisibility(View.GONE);
        ...
        if (player1Score > player2Score) {
            ...
            playLostSound();
            binding.trophyView.setImageResource(R.drawable.lost_game);
            binding.resultText.setText("You Lost...!");
